...
(t/deftest container-test
  (fact "container"
    (let [new-container #(c/start (impl/new-stateful-container {:spec {:c {:zou/constructor map->Foo}}}))]

      ;; lifecycle test
      (fact "start"
        (let [container (new-container)]
          (get-in (sut/system container) [:c :started]) => nil
          (sut/start-system! container) => container
          (get-in (sut/system container) [:c :started]) => true))

      (fact "stop"
        (let [container (new-container)]
          (get-in (sut/system container) [:c :stopped]) => nil
          (sut/stop-system! container) => container
          (get-in (sut/system container) [:c :stopped]) => true))
